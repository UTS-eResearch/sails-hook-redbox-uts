        ...
        post: [
          ...
          // `Email "data publication is staged" notification to FNCI, DM, Supervisor with link to landing page on Staging`
          {
            ...
            options: {
              ...
              to: "<%= record.metadata.contributor_ci.email %>,<%= record.metadata.contributor_data_manager.email %>,<%= sails.config.email.datalibrarian.email %>",
          ...
          // Triggers "Published" Email Notification to FNCI, DM, Collaborators, CC: librarian with RDA link
          {
            ...
            options: {
              ...
              to: "<%= record.metadata.contributor_ci.email %>,<%= record.metadata.contributor_data_manager.email %>,<%= sails.config.email.datalibrarian.email %>,<%= _.isEmpty(record.metadata.creators) ? '' : _.join(_.map(record.metadata.creators, (creator)=>{ return creator.email; }), ',') %>",
      ...
      onUpdate: {
        pre: [
          ...
          // Set the notification state for draft publications
          {
            ...
            options: {
              name: "Set Notification to Draft",
              // when notification is undefined, start with 'draft', so skipping stages will still work (as with the shipped behavior above)
              triggerCondition: "<%= record.workflow.stage == 'draft' && record.notification.state == 'emailed-reviewing'%>",
              flagName: 'notification.state', // the record's path to the notification flag
              flagVal: 'draft', // hard coded value
              saveRecord: false // when true, do metadata update -> false, since this is on a pre-save hook, gets saved anyway
            }
        ...
        post: [
          // push the dataset to the staging web server
          {
            ...
            options: {
              triggerCondition: "<%= record.workflow.stage=='reviewing' %>",
              site: '{{ REDBOX_PORTAL_DATAPUBS_STAGED_NGINX }}'
            }
          },
          {
            function: 'sails.services.emailservice.sendRecordNotification',
            options: {
              triggerCondition: "<%= record.notification.state == 'draft' && record.workflow.stage == 'reviewing'  %>",
              to: "<%= record.metadata.contributor_ci.email %>,<%= record.metadata.contributor_data_manager.email %>,<%= sails.config.email.datalibrarian.email %>",
              subject: "A publication has been staged for review.",
              template: "publicationStaged",
              onNotifySuccess: [
                // `Email "data publication is ready for review" notification to Librarian <%= sails.config.email.datalibrarian.email %> with a link to the data publication record`
                {
                  function: 'sails.services.emailservice.sendRecordNotification',
                  options: {
                    forceRun: true,
          ...
          // Triggers "Published" Email Notification to FNCI, DM, Collaborators, CC: librarian with RDA link
          {
            function: 'sails.services.emailservice.sendRecordNotification',
            options: {
              triggerCondition: "<%= record.notification.state == 'emailed-reviewing' && record.workflow.stage == 'published' %>",
              to: "<%= record.metadata.contributor_ci.email %>,<%= record.metadata.contributor_data_manager.email %>,<%= sails.config.email.datalibrarian.email %>",
              subject: "A publication has been successfully published",
              template: "publicationPublished",
              onNotifySuccess: [
                ...
                },
                {
                  ...
                  options: {
                    forceRun: true,
                    to: "<%= record.metadata.contributor_data_manager.email %>",
                    subject: "A publication has been successfully published",
                    template: "publicationPublished"
                  }
                },
